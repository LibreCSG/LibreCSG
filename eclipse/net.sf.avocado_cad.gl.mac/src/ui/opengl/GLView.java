...
public class GLView implements IGLView {
	...
	private ICSGSolid buildSolidFeat2D3D = null;
	 ...
	 */
	public boolean OPENGL_FRAGMENT_SHADER_GLSL_ENABLED;
	 ...
	 */
	public boolean OPENGL_FRAGMENT_PROGRAM_ARB_ENABLED;

	public GLView(){
	}
	public void setup(Composite comp) {
		GLGlobal.glView = this;
				...
				if (glCanvas.isDisposed()) return;
					...
					if(AvoGlobal.updateGLView){
						...
						if(AvoGlobal.project.getActivePart() != null){
							...
							IPart part = AvoGlobal.project.getActivePart();							
							...
							GLDraw.glDrawSolid(gl,part); 				// draw entire part first...
							GLDraw.glDrawSelectedElements(gl,part); 	// overlay drawing of seleted elements.
							GLDraw.glDrawImportantEdges(gl,part); 		// now draw outlines to emphasize important areas. 

							gl.glLoadIdentity();	// ensure back at identity orientation.							
							ISubPart activeSubPart = AvoGlobal.project.getActiveSubPart();
							// TODO: draw sketches if not consumed and not active
							for(ISubPart subPart : part.getSubParts()) {
								if(subPart != null && subPart != activeSubPart){
									ISketch sketch = subPart.getSketch();
									if(sketch != null && !sketch.isConsumed()){
										// draw the sketch, it is not consumed.
										gl.glPushMatrix();	
										{
											GLDraw.glOrientToPlane(gl, sketch.getSketchPlane());
											gl.glLineWidth(2.0f);
											for(IFeature2D f2D : sketch.getFeature2Ds()) {
												// TODO: HACK, don't build primitives here.. build when created/modified!
												if(f2D.isSelected()){
													buildFeaturePrimList(f2D);
												}
												for(IPrim2D prim : f2D.getPrim2DList()){
													GLDraw.glDraw(gl, prim);
												}
											}		
										}
										gl.glPopMatrix();										
							...
							if(activeSubPart != null){
								ISketch sketch = activeSubPart.getSketch();
								if(sketch != null && !sketch.isConsumed()){
								...
								IBuild build = activeSubPart.getBuild();
								
								if(build != null && build.getParamSet() != null && (AvoGlobal.paramSetToolModels.get(build.getParamSet()) instanceof ToolModelBuild) ){
									// request draw of feat2D3D if active
									sketch = build.getPrimarySketch();
									if(sketch != null && !sketch.isConsumed()){
										// draw regions...
										GLDraw.glOrientToPlane(gl, sketch.getSketchPlane());
										for(IRegion2D region : sketch.getRegion2Ds()) {
											GLDraw.glDrawRegion(gl, region);				
										}
										// only rebuild if necessary.
										if(buildSolidNeedsRebuilt){
											
											ToolModel model = AvoGlobal.paramSetToolModels.get(build.getParamSet());
											
											buildSolidFeat2D3D = ((ToolModelBuild)model).getBuiltSolid(build);
										...
										if(buildSolidFeat2D3D != null){
											...
											if(AvoGlobal.DEBUG_MODE){
												GLDraw.glDrawSolid(gl, buildSolidFeat2D3D);
											}else{
												GLDraw.glDrawWireframe(gl,buildSolidFeat2D3D);
											}
										}
										// TODO: HACK, selecting regions seems to break for non XY plane orientations.
										gl.glLoadIdentity();
										GLDraw.glOrientToPlane(gl, build.getPrimarySketch().getSketchPlane());
										setMouseMatrixToModelview();
									}	
								}
							}
		...
		BackendGlobal.paramEventHandler.addParamListener(new ParamListener(){
			@Override
	...
	private static void buildFeaturePrimList(IFeature2D f2D) {
		ToolModel model = AvoGlobal.paramSetToolModels.get(f2D.getParamSet());
		if(model instanceof ToolModelSketch){
			ToolModelSketch toolModel2D = (ToolModelSketch)model;
			f2D.setPrim2DList(toolModel2D.buildPrim2DList(f2D.getParamSet()));
			f2D.setSelected(f2D.isSelected());
		}
	}
	...
	public void setViewSketch(){
		ISketch sketch = AvoGlobal.project.getActiveSketch();
		if(sketch != null){
