...
public class ToolSketchRectCtrl implements ToolCtrlSketch {
	...
	public void glMouseDrag(double x, double y, double z, MouseEvent e, IParamSet paramSet) {
		IFeature2D feat2D = AvoGlobal.project.getActiveFeat2D();
		if(feat2D != null){
			//
			// get parameter set
			//
			...
			try{
				...
				(new ToolSketchRectModel()).updateDerivedParams((ParamSet)paramSet);
			}catch(Exception ex){
	...
	public void glMouseUp(double x, double y, double z,  MouseEvent e, IParamSet paramSet) {
		...
		if(feat2D != null){
			...
			try{
				...
				IPoint2D ptA = paramSet.getParam("a").getDataPoint2D();
				IPoint2D ptB = paramSet.getParam("b").getDataPoint2D();
				...
				if(ptA.getX() == ptB.getX() || ptA.getY() == ptB.getY()){
					...
					((Sketch)AvoGlobal.project.getActiveSketch()).removeActiveFeat2D();
