...
import net.sf.avocado_cad.model.api.event.ModelListener;
...
public class TreeViewer {
	...
	public TreeViewer(Composite comp, int style){
		...
		BackendGlobal.modelEventHandler.addModelListener(new ModelListener(){
			...
			public void mouseDoubleClick(MouseEvent e) {
				...
				if(tree.getSelection().length > 0){
					...
					if(indxs.length == 2){
						// part selected
						AvoGlobal.project.setActiveGroup(indxs[0]);
						((Group)AvoGlobal.project.getActiveGroup()).setActivePart(indxs[1]);
					...
					if(indxs.length == 3){
						...
						if(indxs[2] == 0){
							...
							IPartMaterial m = AvoGlobal.project.getActivePart().getPartMaterial();
						...
						if(indxs[2] == 1 || indxs[2] == 2 || indxs[2] == 3){
							...
							// sub-part selected.
							ISubPart subpart = ((Part)AvoGlobal.project.getActivePart()).getAtIndex(indxs[2]-4);
							ISketch sketch =  subpart.getSketch();
							if(sketch != null){
								// subpart was a "sketch" tool
								((Part)AvoGlobal.project.getActivePart()).setActiveSubPart(indxs[2]-4);
								AvoGlobal.menuet.setCurrentToolMode(Menuet.MENUET_MODE_SKETCH);
								AvoGlobal.updateGLView = true;
							}
							IBuild feat2D3D = subpart.getBuild();
							if(feat2D3D != null){
							...
							IModify feat3D3D = subpart.getModify();
							if(feat3D3D != null){
	...
	void buildTreeFromAssembly(){
		IProject project = AvoGlobal.project;
		...
		for(int iGroup=0; iGroup < project.getGroupListSize(); iGroup++){
			Group group = (Group) project.getAtIndex(iGroup);
			TreeItem tiGroup;
			...
			for(int iPart=0; iPart<group.getPartListSize(); iPart++){
				...
				for(int iSubPart=0; iSubPart < part.getSubPartListSize(); iSubPart++){
					ISubPart subPart = part.getAtIndex(iSubPart);
					TreeItem tiSubPart;
					...
					Sketch sketch = (Sketch)subPart.getSketch();					
					if(sketch != null){
						...
						for(int iSketch=0; iSketch < sketch.getFeat2DListSize(); iSketch++){
							...
							tiFeat2D.setText(feat2D.getParamSet().getLabel());
						}						
					}
					Build feat2D3D = (Build) subPart.getBuild();
					if(feat2D3D != null){
						if(feat2D3D.getParamSet() != null){
							tiSubPart.setText(feat2D3D.getParamSet().getLabel());
						}else{
							tiSubPart.setText("2Dto3D " + feat2D3D.ID);
						}
					}
					Modify feat3D3D = (Modify)subPart.getModify();
					if(feat3D3D != null){
