...
import net.sf.avocado_cad.model.api.adt.ParamNotCorrectTypeException;
...
public class PCompSelectionList extends ParamComp{
	...
	public PCompSelectionList(Composite parent, int style, IParam p, ParamSet paramSet){
			...
			public void keyPressed(KeyEvent e) {
				if(e.character == SWT.DEL){
					...
					if(lSL.getSelectionIndex() != -1){
						...
						ISelectionList selList = getParamData();
						selList.clearList();
			...
			public void focusGained(FocusEvent e) {
				...
				while(pIter.hasNext()){
					...
					if(!p.equals(param) && p.getType() == ParamType.SelectionList){
						try{
						ISelectionList selList = p.getDataSelectionList();
						if(selList != null){
							selList.setHasFocus(false);
						}
						}catch(Exception ex){
							ex.printStackTrace();
						}
		...
		BackendGlobal.paramEventHandler.addParamListener(paramListener);
		
	}
	
	SelectionList getParamData(){
		try{
			SelectionList data = (SelectionList)param.getDataSelectionList();
			return data;
