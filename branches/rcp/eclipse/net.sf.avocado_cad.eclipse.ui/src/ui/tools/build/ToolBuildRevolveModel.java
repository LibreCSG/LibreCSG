...
public class ToolBuildRevolveModel implements ToolModelBuild{
	...
	public void finalize(ParamSet paramSet) {
		...
		Build feat2D3D = (Build) AvoGlobal.project.getActiveFeat2D3D();
		if(feat2D3D != null){
			...
			if(sketch != null){
				...
				ISketchPlane sp = sketch.getSketchPlane();
				IRotation3D rotation = new Rotation3D(sp.getRotationX(), sp.getRotationY(), sp.getRotationZ());
				ITranslation3D trans = new Translation3D(sp.getOrigin().getX(), sp.getOrigin().getY(), sp.getOrigin().getZ());
				...
				((Part)AvoGlobal.project.getActivePart()).removeActiveSubPart();				
			}			
		}else{
	...
	public boolean paramSetIsValid(IParamSet paramSet) {
		...
		boolean isValid = (	paramSet != null &&
				"Revolve".equals(paramSet.getLabel()) &&
	...
	public ParamSet constructNewParamSet() {
		ParamSet pSet = new ParamSet("Revolve");
		...
		AvoGlobal.paramSetToolModels.put(pSet,new ToolBuildRevolveModel());
		return pSet;
	}
	...
	public CSG_Solid getBuiltSolid(IBuild feat2D3D) {
		IParamSet paramSet = feat2D3D.getParamSet();
		Sketch sketch = (Sketch) feat2D3D.getPrimarySketch();
		...
		if(sketch != null && paramSet != null){					
			try{
				...
				ISelectionList centerLineSel = paramSet.getParam("centerline").getDataSelectionList();
				int faceCounter = 1;
				...
				if(selectionList != null && angle != null && offset != null && 
					...
					if(regNum.length == 2 && sketch.getFeat2DListSize() > Integer.parseInt(regNum[0])){
						...
						if(feat2D != null && feat2D.prim2DList.size() > Integer.parseInt(regNum[1])){
							IPrim2D prim2DCLMaybe = feat2D.prim2DList.get(Integer.parseInt(regNum[1]));
							if(prim2DCLMaybe instanceof Prim2DLine){
								...
								for(int i=0; i<selectionList.getSelectionSize(); i++){
									...
									if(includedRegion != null){
										CSG_Solid newSolid =  getSolidFromRegion(includedRegion, angle, offset, prim2DCenterLine, feat2D3D.getID(), faceCounter);
										...
										for(Region2D cutReg : cutRegions){
											if(cutReg != null){
												CSG_Solid cutSolid = getSolidFromRegion(cutReg, angle, offset, prim2DCenterLine, feat2D3D.getID(), faceCounter);
...
//		solid.addFace(startFace);
		...
		if(pList.size() >= 3){
			...
			IRotation3D preRot = new Rotation3D(0.0, 0.0, zrot);
			ITranslation3D postTrans = new Translation3D(-xoffset, -yoffset, 0.0);
			IRotation3D postRot = new Rotation3D(0.0, 0.0, -zrot);
			ITranslation3D noTrans3D = new Translation3D(0.0, 0.0, 0.0);
			
			// test vertex to see if the normals need to be flipped.
			CSG_Vertex bary = region.getCSGFace().getFaceBarycenter();
			...
			// add first face...
			CSG_Face startFace = region.getCSGFace().getTranslatedCopy(preTrans);
			...
			for(int i=0; i<angleSteps; i++){			
				ICSGVertex lastVert = new CSG_Vertex(pList.getLast(), 0.0);
				ICSGVertex curVert = new CSG_Vertex(pList.getLast(), 0.0).addToVertex(preTrans).getTranslatedRotatedCopy(noTrans3D, preRot);
				...
				while(pIter.hasNext()){
					...
					CSG_Vertex a = (CSG_Vertex)lastVert.getTranslatedRotatedCopy(noTrans3D, new Rotation3D(radOffAngle+angleStep*i, 0.0, 0.0)).getTranslatedRotatedCopy(postTrans, postRot);
					CSG_Vertex b = (CSG_Vertex)lastVert.getTranslatedRotatedCopy(noTrans3D, new Rotation3D(radOffAngle+angleStep*(i+1), 0.0, 0.0)).getTranslatedRotatedCopy(postTrans, postRot);
					CSG_Vertex c = (CSG_Vertex)curVert.getTranslatedRotatedCopy(noTrans3D, new Rotation3D(radOffAngle+angleStep*(i+1), 0.0, 0.0)).getTranslatedRotatedCopy(postTrans, postRot);
					CSG_Vertex d = (CSG_Vertex)curVert.getTranslatedRotatedCopy(noTrans3D, new Rotation3D(radOffAngle+angleStep*i, 0.0, 0.0)).getTranslatedRotatedCopy(postTrans, postRot);
			...
			// close it up! :)
			CSG_Face endFace = region.getCSGFace().getTranslatedCopy(preTrans);
