...
public class ToolBuildExtrudeModel implements ToolModelBuild{
	...
	public boolean paramSetIsValid(IParamSet paramSet) {
		...
		boolean isValid = (	paramSet != null &&
							"Extrude".equals(paramSet.getLabel()) &&
	...
	public ParamSet constructNewParamSet() {
		...
		AvoGlobal.paramSetToolModels.put(pSet,new ToolBuildExtrudeModel());
		return pSet;
	}
				...
				if(selectionList != null && height != null){
					...
					for(int i=0; i<selectionList.getSelectionSize(); i++){
						...
						if(includedRegion != null){
							CSG_Solid newSolid =  getSolidFromRegion(includedRegion, height, feat2D3D.getID(), faceCounter);
							...
							for(Region2D cutReg : cutRegions){
								if(cutReg != null){
									CSG_Solid cutSolid = getSolidFromRegion(cutReg, height, feat2D3D.getID(), faceCounter);
	...
	private CSG_Solid getSolidFromRegion(Region2D region, double height, int ID, int faceCounter){
		CSG_Solid solid = new CSG_Solid();
		// top Face
		CSG_Face topFace = region.getCSGFace().getTranslatedCopy(new CSG_Vertex(0.0, 0.0, height));
		...
		// bottom face
		CSG_Face botFace = region.getCSGFace();
		if(height < 0.0){ // make sure normal points correct way (outward).
		...
		while(iter.hasNext()){
			IPrim2D prim2D = iter.next();
			if(prim2D instanceof Prim2DArc){
				Prim2DArc arc = (Prim2DArc)prim2D;
				ICSGVertex arcCenterPt = new CSG_Vertex(arc.getArcCenterPoint(), 0.0);
	...
	private void addSimilarFacesByPointList(LinkedList<? extends IPoint2D> pointList, ModRef_Plane planeRef, ModRef_Cylinder cylRef, double height, CSG_Solid solid){
		CSG_Vertex lastVert        = null;
		CSG_Vertex lastVertExtrude = null;
		for(IPoint2D pt : pointList){
			if(lastVert == null){
