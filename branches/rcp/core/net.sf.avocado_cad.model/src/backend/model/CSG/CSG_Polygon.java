...
*/
public class CSG_Polygon implements ICSGPolygon {
	...
	private void removeAdjacentDuplicates(){
		ICSGVertex lastV = vertices.getLast();
	...
	public double getArea(){
		...
		ICSGVertex barycenter = getBarycenterVertex();
	...
	public List<CSG_Vertex> getVertices(){
	...
	private CSG_Plane computePlane(){
		...
		ICSGVertex v1 = vertices.get(0);
		ICSGVertex v2 = vertices.get(1);
		ICSGVertex v3 = vertices.get(2);
	 ...
	 */
	public boolean vertexIsInsidePolygon(ICSGVertex testVert){
	 ...
	 */
	public boolean vertexIsInsideOrOnEdgeOfPolygon(ICSGVertex testVert){
		CSG_Vertex lastVert = vertices.get(vertices.size()-1);
		for(CSG_Vertex vert : vertices){
			CSG_Vertex vertDiff = vert.subFromVertex(lastVert);
			if(getPlane() == null || getPlane().getNormal() == null){
	...
	public ICSGPolygon getClosestJoiningPolygon(CSG_Polygon poly2){
		ICSGPolygon polyJoin = null;
		double distance = Double.MAX_VALUE;
		Iterator<CSG_Vertex> vertIter = vertices.iterator();
		while(vertIter.hasNext()){
