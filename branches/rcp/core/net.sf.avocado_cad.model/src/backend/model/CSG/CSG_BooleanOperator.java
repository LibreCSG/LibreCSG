...
public class CSG_BooleanOperator {
	...
	public static CSG_Solid Intersection(CSG_Solid solidA, CSG_Solid solidB){
		...
		addPolygonsFromSolidToSolid(solidAClone, POLY_TYPE.POLY_INSIDE, newSolid, false);
		addPolygonsFromSolidToSolid(solidAClone, POLY_TYPE.POLY_SAME, newSolid, false);
		addPolygonsFromSolidToSolid(solidBClone, POLY_TYPE.POLY_INSIDE, newSolid, false);
	...
	public static CSG_Solid Union(CSG_Solid solidA, CSG_Solid solidB){
		...
		classifySolidAPolysInSolidB(solidBClone, solidAClone);
		CSG_Solid newSolid = new CSG_Solid();
		addPolygonsFromSolidToSolid(solidAClone, POLY_TYPE.POLY_OUTSIDE, newSolid, false);
		addPolygonsFromSolidToSolid(solidAClone, POLY_TYPE.POLY_SAME, newSolid, false);
		addPolygonsFromSolidToSolid(solidBClone, POLY_TYPE.POLY_OUTSIDE, newSolid, false);
		System.out.println("Computing Mating Edges!");
		newSolid.computeAllMatingEdgeLines();
		return newSolid;
	}
	...
	public static CSG_Solid Subtraction(CSG_Solid solidA, CSG_Solid solidB){
		...
		addPolygonsFromSolidToSolid(solidBClone, POLY_TYPE.POLY_INSIDE, newSolid, true);
		System.out.println("Computing Mating Edges!");
		newSolid.computeAllMatingEdgeLines();
		return newSolid;
	}
	...
	private static void splitSolidABySolidB(CSG_Solid sA, CSG_Solid sB){
		...
		if(sB.getBounds().overlapsBounds(sA.getBounds())){
			// ( 2) for each faceB in solidB
			for(CSG_Face faceB : sB.getFaces()) {
				// ( 3) if(extent of faceB overlaps solidA)
				if(faceB.getBounds().overlapsBounds(sA.getBounds())){
					...
					while(polyIterB.hasNext()){
						...
						if(polyB.getBounds().overlapsBounds(sA.getBounds())){
							// ( 6) for each faceA in solidA
							for(CSG_Face faceA : sA.getFaces()) {
								// ( 7) if(extent of polygonB overlaps faceA)
	...
	private static void classifyPolygonAInSolidB(CSG_Polygon polyA, CSG_Solid solidB){
		...
		while(!castWasSuccessful){
			...
			for(CSG_Face faceB : solidB.getFaces()) {
				Iterator<CSG_Polygon> polyIterB = faceB.getPolygonIterator();
		...
		if(closestPolyB == null){
			...
			polyA.type = POLY_TYPE.POLY_OUTSIDE;
		}else{
			...
			if(closestDistIsZero){
				if(dotProduct > TOL){
					polyA.type = POLY_TYPE.POLY_SAME;
				}else{
					if(dotProduct < -TOL){
						polyA.type = POLY_TYPE.POLY_OPPOSITE;
					}
				}
			}else{
				if(dotProduct > TOL){
					polyA.type = POLY_TYPE.POLY_INSIDE;
				}else{
					if(dotProduct < -TOL){
						polyA.type = POLY_TYPE.POLY_OUTSIDE;
					}
	 ...
	 * @param reverseOrder
	 */
	private static void addPolygonsFromSolidToSolid(CSG_Solid inputSolid, POLY_TYPE type, CSG_Solid solidToModify, boolean reverseOrder){
		for(CSG_Face face : inputSolid.getFaces()) {
