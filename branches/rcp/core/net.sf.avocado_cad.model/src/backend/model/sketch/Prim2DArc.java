...
* @created Feb. 2007
*/
public class Prim2DArc extends Prim2D implements IPrim2DArc {
	...
	public Point2D intersectsArc(IPrim2DArc arc) {
		double arcradius = arc.getArcRadius();
		IPoint2D arccenter = arc.getArcCenterPoint();
		
		double dist = this.center.computeDist(arc.getArcCenterPoint());
		if(dist >= this.radius + arc.getArcRadius()){
			// arcs are too far apart.. there is no way for them to intersect.
			return null;
		}
		
		double a = (this.radius*this.radius - arcradius*arcradius + dist*dist) / (2*dist);
		double h = Math.sqrt(this.radius*this.radius - a*a);
		IPoint2D mid = this.center.addPt((arccenter.subPt(this.center)).getScaledPt(a/dist));
		double pt1X = mid.getX() + (h/dist)*(arccenter.getY()-this.center.getY());
		double pt1Y = mid.getY() - (h/dist)*(arccenter.getX()-this.center.getX());
		Point2D pt1 = new Point2D(pt1X, pt1Y);
		double pt2X = mid.getX() - (h/dist)*(arccenter.getY()-this.center.getY());
		double pt2Y = mid.getY() + (h/dist)*(arccenter.getX()-this.center.getX());
		...
		if(distTo1 < Geometry2D.epsilon){
			...
			if(!pt1.equalsPt(arc.getPtA()) && !pt1.equalsPt(arc.getPtB())){
				return pt1;
			}
		}
		...
		if(distTo2 < Geometry2D.epsilon){
			// could be good, just make sure it's not an end point.
			if(!pt2.equalsPt(arc.getPtA()) && !pt2.equalsPt(arc.getPtB())){
				return pt2;
			}
		}
		return null;
	...
	public Point2D intersectsLine(IPrim2DLine ln) {		
		// Intersection code of line with arc	
		IPoint2D ptA = ln.getPtA();
		IPoint2D ptB = ln.getPtB();
		IPoint2D ptC = center;		
		double r = radius;
	...
	public double distFromPrim(IPoint2D pt) {
	...
	public Point2D intersect(IPrim2D anyPrim2D) {
	...
	public PrimPair2D splitPrimAtPoint(IPoint2D pt) {
