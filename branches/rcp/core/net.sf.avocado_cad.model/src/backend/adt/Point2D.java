...
public class Point2D implements IPoint2D {
	 ...
	 * @return
	 */
	public Point2D addPt(IPoint2D p2){
	 ...
	 * @return
	 */
	public Point2D subPt(IPoint2D p2){
		return new Point2D(x-p2.getX(), y-p2.getY());
	}
	...
	public boolean equalsPt(IPoint2D p2){
		return ((x > (p2.getX()-Geometry2D.epsilon) && (x < (p2.getX()+Geometry2D.epsilon))) && 
				(y > (p2.getY()-Geometry2D.epsilon) && (y < (p2.getY()+Geometry2D.epsilon))));
	}
	 ...
	 */
	public double getCartesianAngle(IPoint2D ptB){
