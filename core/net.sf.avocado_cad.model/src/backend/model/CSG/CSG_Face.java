 ...
 */
public class CSG_Face implements ICSGFace {
	 ...
	 */
	public boolean vertexIsInsideFace(ICSGVertex vert){
		for(CSG_Polygon poly : polygons){
	...
	public Collection<CSG_Polygon> getPolygons() {
		return polygons;
	}
	/**
	 ...
	 */
	public ICSGVertex getFirstPolygonBarycenter(){
		double x = 0.0;
		double y = 0.0;
		double z = 0.0;
		int totalVertices = 0;
	 ...
	 */
	public double distFromVertexToFacePlane(ICSGVertex v){
			...
			for(int n=0; n<polys.size(); n++){
				...
				if(isFirstTime){
					...
					for(int i=0; i<polyVerts; i++){
						ICSGVertex pVertA = poly.getVertAtModIndex(i);
						for(int j=0; j<perim.size(); j++){
							...
							if(pVertA.getDistBetweenVertices(ptA) < TOL){
								...
								ICSGVertex pVertB = poly.getVertAtModIndex(i+1);
	...
	public LinkedList<CSG_Vertex> getMatingEdgeLines() {
		return matingEdgeLines;
	}
