...
public class Region2DList extends LinkedList<Region2D>{
	...
	private void smartSearchForValidCycle(Prim2D startPrim, Prim2DList prunedPrims, IPrim2D extraCheck){
		...
		while(!newCycle.isValidCycle()){
			...
			for(Prim2D primB : prunedPrims){
				if(!newCycle.contains(primB) && primB != extraCheck){
					IPoint2D otherPoint = primB.hasPtGetOther(conPt);
					if(otherPoint != null){
