...
* @created Feb. 2007
*/
public class Prim2DLine extends Prim2D implements IPrim2DLine {
	
	public Prim2DLine(IPoint2D ptA, IPoint2D ptB){
		if(ptA == null || ptB == null){
			ptA = new Point2D(0.0, 0.0);
			ptB = new Point2D(0.0, 0.0);
			System.out.println("prim2DLine constructed with a null end point!");
		}
		this.ptA = (Point2D)ptA;
		this.ptB = (Point2D)ptB;
	}
	

	public Point2D intersectsArc(IPrim2DArc arc) {			
		// Intersection code of arc with line		
		IPoint2D ptC = arc.getArcCenterPoint();		
		double r = arc.getArcRadius();
		...
		double posAngleDiff = (sCircAngle - arc.getStartAngle() + 360.0)%360.0;
		
		// the epsilons "shorten" the acceptable window for the primative that will be modified
		// and the epsilons "widen" the window for "this" primative to ensure checking if very close.
		if(posAngleDiff <= Geometry2D.epsilon || posAngleDiff >= (arc.getArcAngle() - Geometry2D.epsilon)){
	...
	public Point2D intersectsLine(IPrim2DLine ln) {
		IPoint2D ptC = ln.getPtA();
		IPoint2D ptD = ln.getPtB();
